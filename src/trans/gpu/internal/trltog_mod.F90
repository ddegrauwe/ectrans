...
MODULE TRLTOG_MOD
 ...
 &                            PGP,PGPUV,PGP3A,PGP3B,PGP2,LDTRANSPOSED)
  ...
  REAL(KIND=JPRBT) :: PGLAT_C;
  INTEGER(KIND=JPIM) :: FIELD_SWITCH,OUT_B,OUT_E,IPOS_OFFSET;
...
#define TRLTOG_DTYPE MPI_DOUBLE_PRECISION
  ...
  IF( IRECVTOT(MYPROC) > 0 )THEN
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK,PGLAT_C) &
        ...
        DO JFLD=1,IFLDS
          DO JKL=1,JK_MAX
            ...
            IF(IFIRST>0 .AND. JK<=ILAST) THEN
              ...
              PGLAT_C=PGLAT(KINDEX(IPOS),JFLD)
              IF(LLINDER) THEN
                IFLD = KPTRGP(JFLD)                
				        PGP(JK,IFLD,JBLK) = PGLAT_C
				      ELSE
                IFLD = IFLDOFF(JFLD)                
				        PGP(JK,IFLD,JBLK) = PGLAT_C
				      ENDIF
    ...
    ELSE    
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP GANG DEFAULT(NONE) PRIVATE(IFLD,JFLD) &
      !$ACC&     COPYIN(IGPTROFF,IFLDOFF,JK_MAX)&
      !$ACC&     COPYIN(NGPBLKS,IFLDS,IGPTRSEND,MYSETW,INDOFF,MYPROC, &
      !$ACC&            LLUV,PGPUV,IUVLEVS,IUVPARS,LLGP2,PGP2,IGP2PARS, &
      ...
      !$ACC&      PRIVATE(FIELD_SWITCH,OUT_B,OUT_E)&
      !$ACC& PRIVATE(IPOS,IFIRST,ILAST,JK) &
      !$ACC& PRIVATE(PGLAT_C,JKL,JBLK,IPOS_OFFSET) &	    
        DO JBLK=1,NGPBLKS         
             IPOS_OFFSET=INDOFF(MYPROC)+IGPTROFF(JBLK)                       
             !$ACC LOOP VECTOR TILE(128,128)
              DO JFLD=1,IFLDS
              DO JKL=1,JK_MAX                        
              IFIRST = IGPTRSEND(1,JBLK,MYSETW)
              IF(IFIRST>0) THEN
                ...
                ILAST = IGPTRSEND(2,JBLK,MYSETW)
                IF(JK<=ILAST) THEN                                    
                  IPOS = IPOS_OFFSET+JKL           
                  IFLD = IFLDOFF(JFLD)   
                  PGLAT_C=PGLAT(KINDEX(IPOS),JFLD)
                    
                    IF(LLUV(IFLD)) THEN	
			                  PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK) = PGLAT_C
                    ELSEIF(LLGP2(IFLD)) THEN			      
	                      PGP2(JK,IGP2PARS(IFLD),JBLK)= PGLAT_C
                    ELSEIF(LLGP3A(IFLD)) THEN			                  
                        PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK)=PGLAT_C
                    ELSEIF(LLGP3B(IFLD)) THEN			  
			                  PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK)=PGLAT_C
                    ENDIF
              ENDIF
            ENDIF
          ENDDO
