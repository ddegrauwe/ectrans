...
IF(IUBOUND(1) < NPROMA) THEN
  WRITE(NOUT,*)'GPNORM_GPNORM_TRANS_CTLTRANS:FIRST DIM. OF PGP TOO SMALL ',IUBOUND(1),NPROMA
...
CALL GSTATS(1429,0)
IF( IF_FS > 0 )THEN
  ! daand: move to cpu version for now.
  !$acc data present(PREEL_REAL)
  !$acc update host(PREEL_REAL)
  !$acc end data
...
! FIRST DO SUMS IN EACH FULL LATITUDE
  ...
  !*$ACC KERNELS private(JGL,IGL,JL,V) default(none)
  DO JGL=1,D%NDGL_FS
  ...
  DO JGL=IBEG,IEND
    IGL = D_NPTRLS(MYSETW) + JGL - 1
    DO JF=1,IF_FS
      ZAVE(JF,JGL)=ZAVE(JF,JGL)*PW(IGL)/G_NLOEN(IGL)
