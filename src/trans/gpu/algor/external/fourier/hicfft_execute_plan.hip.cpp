...
/***
template <class Type, hipfftType Direction>
void execute_fft_new_1(typename Type::real *data_real, typename Type::cmplx *data_complex,
  ...
  if (fftPlans == fftPlansCache.end()) {
    ...
    printf("creating new fft plan for kfield = %i\n",kfield); fflush(stdout);
	...
	printf("reusing existing fft plan for kfield = %i\n",kfield); fflush(stdout);
  }
	...
	if constexpr(Direction == HIPFFT_R2C)
	  fftSafeCall(hipfftExecR2C(fftPlans->second[0], data_real_l, data_complex_l));
	else if constexpr(Direction == HIPFFT_C2R)
	  fftSafeCall(hipfftExecC2R(fftPlans->second[0], data_complex_l, data_real_l));
	else if constexpr(Direction == HIPFFT_D2Z)
	  fftSafeCall(hipfftExecD2Z(fftPlans->second[0], data_real_l, data_complex_l));
	else if constexpr(Direction == HIPFFT_Z2D)
	  fftSafeCall(hipfftExecZ2D(fftPlans->second[0], data_complex_l, data_real_l));
...
void execute_fft_new(typename Type::real *data_real, typename Type::cmplx *data_complex,
  ...
  if ( nfft == 1 ) {
    // LAM case: nfft=1 (all fft's have same length); doesn't require graphs.
	...
	//*      get_fft_plan_cache<Type, Direction>(); // kfield -> handles
	  ...
	  if (graph == graphCache.end()) {
		  ...
		  printf("creating new graph for kfield = %i\n",kfield); fflush(stdout);
		  ...
		  printf("reusing existing graph for kfield = %i\n",kfield); fflush(stdout);
	  }
	  
	  printf("launching graph now!\n"); fflush(stdout);
